...
describe('deck edit batch flow', () => {
  ...
  it('restores only pages rejected by final deck-level validation', async () => {
    const fixture = await createFixture(2)
    const results = await executeDeckEditBatchFlow({
      pageRefs: fixture.pageRefs,
      indexPath: fixture.indexPath,
      originalUserMessage: '统一整套视觉',
      runId: 'run-deck-quality-final',
      appLocale: 'zh',
      launchStaggerMs: 0,
      emit: () => undefined,
      validateChangedPages: () => [],
      validateFinalResults: async () => [
        { pageId: 'page-2', reason: '[deck-font-system-drift] core font changed' }
      ],
      buildRetryMessage: () => null,
      runPageAttempt: async ({ pageId }) => {
        const page = fixture.pageRefs.find((item) => item.pageId === pageId)!
        await fs.promises.writeFile(
          page.htmlPath,
          validPageHtml(page.pageId, `after-${page.pageId}`),
          'utf-8'
        )
      }
    })
    ...
    expect(results).toEqual([
      expect.objectContaining({ status: 'completed', pageId: 'page-1' }),
      expect.objectContaining({
        status: 'failed',
        pageId: 'page-2',
        reason: '[deck-font-system-drift] core font changed'
      })
    ])
    await expect(fs.promises.readFile(fixture.pageRefs[0].htmlPath, 'utf-8')).resolves.toContain(
      'after-page-1'
    )
    await expect(fs.promises.readFile(fixture.pageRefs[1].htmlPath, 'utf-8')).resolves.toContain(
      'before-page-2'
    )
  })
  ...
  it('reports the page failure reason before retrying and advances progress after recovery', async () => {
    ...
    expect(emitted).toContainEqual(expect.objectContaining({ label: 'P1 重试成功', progress: 90 }))
  })
  ...
  it('logs a heartbeat without emitting user-visible progress while a model request is silent', async () => {
    ...
    expect(emitted).not.toContainEqual(expect.objectContaining({ label: 'P1 正在等待模型响应' }))
