...
describe('style case filters', () => {
  ...
  it('powers the styles page filter: a style is reachable from each of its tags', () => {
    const options = [
      { id: 'sharp-mono', label: '锐利黑白', styleCase: '技术分享、教学科普、产品发布' },
    ...
    // 技术分享 同时命中 sharp-mono 和 minimal-white
  ...
  it('filters styles by keyword over name, description and use cases', () => {
    const options = [
      { id: 'sharp-mono', label: '锐利黑白', description: '深色技术风', styleCase: '技术分享、教学科普' },
      { id: 'gold-ivory', label: '金象牙', description: '奢华品牌', styleCase: '餐饮美食、品牌营销' },
      { id: 'blue-white-chart', label: '蓝白商务图表', description: '商业数据汇报', styleCase: '商业汇报' }
    ]

    // 命中名称
    expect(filterByStyleKeyword(options, '锐利').map((o) => o.id)).toEqual(['sharp-mono'])
    // 命中描述
    expect(filterByStyleKeyword(options, '奢华').map((o) => o.id)).toEqual(['gold-ivory'])
    // 命中用途
    expect(filterByStyleKeyword(options, '商业').map((o) => o.id)).toEqual(['blue-white-chart'])
    // 大小写无关 + 首尾空格容错；未命中返回空
