...
describe('initializeSkills', () => {
  ...
  it('installs bundled system skills into installed root and writes manifest', async () => {
    ...
    await makeSkill(bundled, 'amy-ppt-data-anim', '1.0.0')
    ...
    expect(manifest.skills['amy-ppt-data-anim']).toMatchObject({
      version: '1.0.0',
      source: 'builtin',
    })
    await expect(
      readFile(path.join(installed, 'system', 'amy-ppt-data-anim', 'SKILL.md'), 'utf8')
  ...
  it('keeps installed skills readable through the DeepAgents composite route', async () => {
    ...
    const bundled = path.join(tmp, 'bundled')
    const installed = path.join(tmp, 'installed')
    await makeSkill(bundled, 'amy-ppt-data-anim', '1.0.0', '# Routed Skill\n')
    await makeSkill(bundled, 'amy-ppt-chart', '1.0.0', '# Routed Chart Skill\n')
    ...
    const dataAnimSkillPath = `${skillSource}amy-ppt-data-anim/SKILL.md`
    const chartSkillPath = `${skillSource}amy-ppt-chart/SKILL.md`
    const listed = await backend.ls(skillSource)
    expect(listed.error).toBeUndefined()
    expect(listed.files?.map((file) => file.path)).toContain(
      `${skillSource}amy-ppt-data-anim/`
    )
    expect(listed.files?.map((file) => file.path)).toContain(
      `${skillSource}amy-ppt-chart/`
    )
  ...
  it('upgrades system skills by numeric version and marks removed bundled skills', async () => {
    ...
    const bundled = path.join(tmp, 'bundled')
    const installed = path.join(tmp, 'installed')
    await makeSkill(bundled, 'amy-ppt-data-anim', '1.0.0', 'old body\n')
    ...
    await makeSkill(upgradedBundled, 'amy-ppt-data-anim', '1.10.0', 'new body\n')
    ...
    expect(upgraded.manifest.skills['amy-ppt-data-anim']).toMatchObject({
      version: '1.10.0',
    })
    await expect(
      readFile(path.join(installed, 'system', 'amy-ppt-data-anim', 'SKILL.md'), 'utf8')
