# Amy-PPT Layout — Pattern Catalog
...
- **Input shape**: One strong conclusion supported by 2–4 pieces of proof, implications, or reasons. Low-medium density.
- **Structure recipe**: Give the conclusion a dominant statement zone, then place proof in a narrow evidence rail, aligned rows, or one lower band. The reading path is conclusion → proof → implication; proof modules remain subordinate and share one alignment system.
- **Budget rule**: The conclusion owns roughly one third to one half of the usable visual weight. Proof uses the remaining space as concise evidence rather than equal hero cards; an optional one-line implication may anchor the bottom.
- **Failure signs**: The conclusion and every proof item have equal card weight, or proof repeats the title in different words → restore one dominant statement, merge repeated evidence, and keep proof visually subordinate.
...
- **Input shape**: One decisive metric plus 2–3 compact facts explaining its basis, change, benchmark, or consequence. Low-medium density.
- **Structure recipe**: Hero number and label occupy the dominant zone. Evidence sits in a side rail or a single attached band, ordered as basis → comparison → meaning. Each evidence module uses a short module title and one line of body copy.
- **Budget rule**: The metric remains the first and largest object; evidence occupies no more than the remaining half of the composition and never becomes a second dashboard row.
- **Failure signs**: Several numbers compete at the same scale, or evidence becomes a grid of KPI cards → choose one hero metric, reduce supporting figures to compact annotations, and make their relationship explicit.
...
- **Input shape**: 3–6 options, states, or examples that differ along one meaningful continuum such as low → high, current → future, or simple → advanced. Medium density.
- **Structure recipe**: One strong horizontal or vertical axis establishes the scale. Position labeled modules along it according to meaning rather than equal card slots; each module contains a short title and at most one explanatory line.
- **Budget rule**: The axis and labels form one coherent middle-zone composition. Leave enough separation for labels to remain readable, and use a small conclusion near the terminal end only when it adds interpretation.
- **Failure signs**: Items are evenly spaced although their meaning is not, labels overlap, or each marker expands into a large card → restore semantic positioning, stagger labels, and keep modules compact.
...
- **Input shape**: One central proposition connected to 4–8 related capabilities, influences, audiences, or outcomes without a strict sequence. Medium density.
- **Structure recipe**: Put the central proposition in a clear middle zone and distribute related modules around it on a balanced radial grid. Connectors are decorative and remain behind text; each outer module uses a short module title plus one line.
- **Budget rule**: The center stays visually dominant and outer modules use the middle canvas rather than hugging the top edge. Reserve clear radial gaps so labels and connectors never collide.
- **Failure signs**: The result becomes an equal card grid, connectors cross through labels, or the center is no stronger than satellites → strengthen the center zone, reduce outer copy, and reposition connectors behind modules.
...
- **Input shape**: 3–5 points with a clear order, progression, maturity, priority, or hierarchy. Medium density.
- **Structure recipe**: Use a title region followed by 3–5 horizontal flat information strips. Each strip contains an icon or number, a module second-level title, and 1–2 body lines. Offset successive strips horizontally by 48–72px to create a diagonal reading path while keeping text in normal grid/flex flow.
- **Budget rule**: On 1600×900, each strip is approximately 92–108px high with 16–22px vertical gaps, a 48–56px icon/number zone, and an 8–10px module-title/body gap. Keep every strip inside the safe area; reduce the offset before reducing readable width.
- **Failure signs**: Content has no sequence but is forced into a staircase; body copy exceeds two lines; progressive offsets make later strips too narrow; or large radii, buttons, and badges turn the structure into a Web feature list.

### `diagonal-progression`

- **Input shape**: 3–5 stages where movement, growth, escalation, or transformation matters more than detailed step instructions. Low-medium density.
- **Structure recipe**: Arrange stages along one diagonal path from starting state to destination. Use scale, position, or connected anchors to show progression; each stage has a concise module title and one supporting line. Keep all readable text horizontal.
- **Budget rule**: The diagonal uses both axes and spans the middle of the canvas. Stage modules stay compact, connectors remain behind content, and the final destination receives modest visual emphasis.
- **Failure signs**: A rotated decoration pretends to be progression, text is tilted, stages collide near the destination, or the page is only a slanted row of equal cards → preserve horizontal text, redistribute anchors, and make the progression relationship explicit.
...
> Standard mode does **not** generate images. Prefer an existing imported image, screenshot, or diagram. Create a clean replaceable semantic placeholder only when the User PPT Composition Profile explicitly allows it and the composition truly needs an image; never reserve image slots mechanically or invent a fake photo.
...
- **Input shape**: One existing image establishes a scene, product, person, or moment, while a concise claim explains why it matters. Low-medium density.
- **Structure recipe**: Let the image occupy a full-height or full-bleed dominant zone. Place a compact title/claim and at most two annotations in deliberate negative space or an attached text rail; the reading path is image → claim → detail.
- **Budget rule**: The image carries most of the visual weight and receives an intentional crop. Text remains compact and inside the safe area; annotations never cover the image's essential subject.
- **Failure signs**: Text becomes a large card floating over the subject, the image is a decorative thumbnail, or several unrelated images form a gallery → restore one narrative image, protect its focal subject, and reduce copy to the slide's core meaning.
...
- **staircase** — a light offset mechanic for patterns other than `staircase-strips`; modules use incrementing `ml-*` / `pl-*` to create a diagonal reading flow. Use the formal `staircase-strips` pattern when the slide is specifically a sequence of flat horizontal information strips.
