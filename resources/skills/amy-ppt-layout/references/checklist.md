# Amy-PPT Layout — Structural Self-check
...
- Standard mode emits `data-img-slot` / `data-image-slot` / `script[data-img-intent]` or an image job. A replaceable semantic placeholder is allowed only when the User PPT Composition Profile explicitly permits it and the chosen composition truly requires an image.
