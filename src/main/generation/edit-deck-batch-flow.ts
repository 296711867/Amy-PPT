...
export type DeckEditFinalValidationFailure = {
  pageId: string
  reason: string
}
...
export type ExecuteDeckEditBatchFlowArgs = {
  ...
  validateFinalResults?: (
    completedResults: DeckEditCompletedBatch[]
  ) => Promise<DeckEditFinalValidationFailure[]>
...
export async function executeDeckEditBatchFlow(
  ...
  const emitPageProgress = (page: DeckEditBatchPageRef, label: string, detail?: string): void => {
  ...
  try {
    ...
    if (args.validateFinalResults) {
      const completedResults = results.filter(
        (result): result is DeckEditCompletedBatch => result.status === 'completed'
      )
      const finalFailures = await args.validateFinalResults(completedResults)
      const failureByPageId = new Map(finalFailures.map((failure) => [failure.pageId, failure]))
      for (let index = 0; index < results.length; index += 1) {
        const result = results[index]
        if (result.status !== 'completed') continue
        const failure = failureByPageId.get(result.pageId)
        if (!failure) continue
        const page = args.pageRefs.find((candidate) => candidate.pageId === result.pageId)
        if (page) {
          await restoreFileSnapshot(
            page.htmlPath,
            operationSnapshot.pages.get(page.pageId) || { exists: false, content: '' }
          )
        }
        results[index] = {
          status: 'failed',
