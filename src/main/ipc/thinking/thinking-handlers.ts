...
import { resolveThinkingHistoryStoragePath } from './storage-configuration'
...
export function registerThinkingHandlers(ctx: IpcContext): void {
  const { db, resolveStoragePath } = ctx
  ...
  ipcMain.handle('thinking:getLatestWorkspace', async () => {
    const storagePath = await resolveThinkingHistoryStoragePath({
      readStoragePath: () => db.getSetting<string>('storage_path'),
      resolveStoragePath
    })
