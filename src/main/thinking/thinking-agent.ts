...
function readMessageField(record: Record<string, unknown>, key: string): unknown {
  if (record[key] !== undefined) return record[key]
  const kwargs = getObject(record.kwargs)
  if (kwargs?.[key] !== undefined) return kwargs[key]
  return getObject(record.lc_kwargs)?.[key]
}
...
async function collectAgentReply(
  ...
  onThinkingEvent?: ThinkingEventHandler
): Promise<{
  replyText: string
...
function extractAndEmitToolEvents(
  ...
  const visit = (value: unknown): void => {
    ...
    if ((role === 'tool' || toolCallId) && toolCallId) {
      const name = String(readMessageField(record, 'name') || 'tool').trim()
      const key = `tr:${toolCallId}:${name}`
      if (!seen.has(key)) {
        seen.add(key)
        onThinkingEvent({
          id: toolCallId,
          type: 'tool_result',
          toolName: name,
          summary: summarizeToolResult(name)
        })
      }
    }
    ...
    if (Array.isArray(toolCalls)) {
      for (const call of toolCalls) {
        ...
        if (name && id && !seen.has(key)) {
          ...
          if (!summary) continue
          onThinkingEvent({ id, type: 'tool_call', toolName: name, summary })
        }
      }
    }
    for (const nested of Object.values(record)) {
...
function summarizeToolResult(toolName: string): string {
  if (toolName === 'read_file') return '资料读取完成'
  if (toolName === 'grep') return '相关内容定位完成'
  if (toolName === 'update_thinking_document') return 'PPT 方案已更新'
  if (toolName === 'update_context_document') return '需求和关键信息已记录'
  return '步骤已完成'
}
...
type ThinkingEventHandler = (event: {
  id?: string
  type: 'tool_call' | 'tool_result' | 'phase_running' | 'phase_completed' | 'phase_failed'
  toolName: string
  summary: string
}) => void
...
export async function runThinkingChat(args: RunThinkingChatArgs): Promise<ThinkingChatResult> {
  ...
  let replyText = ''
  let latestAssistantStateText = ''
  let recoveredFinalReplyError = false
  ...
  try {
    ...
    if (canRecoverThinkingStreamError(err, runtime.workflowState)) {
      recoveredFinalReplyError = true
      await finalizeStagedThinkingIfNeeded({
        runtime,
        thinkingId,
        reason: 'response-format-recovery'
      })
      onThinkingEvent?.({
        id: 'analysis',
        type: 'phase_completed',
      ...
      onThinkingEvent?.({
        id: 'reply-recovery',
        type: 'phase_completed',
        toolName: 'reply-recovery',
        summary: '最终回复格式异常，已根据保存结果自动恢复'
      })
      log.warn('[thinking:agent] recovered response format error after workflow update', {
        thinkingId,
        contextUpdated: runtime.workflowState.contextUpdated,
        thinkingUpdated: runtime.workflowState.thinkingUpdated,
        thinkingStaged: runtime.workflowState.thinkingStaged
      })
    } else {
  ...
  if (!replyText || recoveredFinalReplyError) {
