...
export function buildEditAgentSystemPrompt(
  ...
  const prompt = isContainerScopeEdit
    ? buildContainerEditPrompt(styleId, context)
    : hasSelector
      ? buildSelectorEditPrompt(styleId, context)
      : isDeckScopeEdit
        ? buildDeckEditPrompt(styleId, context)
        : buildSinglePageEditPrompt(styleId, context)
