...
export function createSessionBoundDeckTools(context: SessionDeckGenerationContext): unknown[] {
  ...
  const searchIconsTool = tool(
    async ({ query, limit }) => {
      ...
      if (results.length === 0) {
        return `未找到匹配 "${query}" 的图标。图标库共 ${getIconCount()} 个，换关键词试（如 arrow/chart/user/star/trending）。`
      }
      return `匹配 "${query}" 的图标（${results.length} 个，用 <svg data-icon="id" class="..."/> 引用，系统自动替换成 SVG）：\n${results
        .map((r) => `- ${r.id}${r.label ? ` ${r.label}` : ''}`)
        .join('\n')}`
    },
    {
      name: 'search_icons',
      description:
        '查找可用图标 id（lucide 图标库共 1683 个）。需要图标但不确定 id 时调用，传中英文关键词（如"增长"/"箭头"/"用户"/"chart"）。返回匹配 id 清单，用 <svg data-icon="id" class="..."/> 引用即可，系统落盘时自动替换成 SVG。',
      schema: z.object({
        query: z.string().describe('搜索关键词，中英文均可，如 "增长" "箭头" "chart" "用户"'),
        limit: z.number().optional().describe('返回上限，默认 20')
      })
    }
  )
          ...
          tool(
            async ({ type, durationMs }, config) => {
              ...
              if (result.status === 'invalid') {
                ...
                throw new Error(`index.html 验证失败: ${result.errors.join('; ')}`)
              }
      ...
      async (_input, config) => {
        if (isContainerScopeEdit) {
          ...
          if (verification.status === 'invalid') {
            return `验证失败：index.html 结构不完整：${verification.errors.join('; ')}`
          }
          emitNormalizedToolStatus(config, {
