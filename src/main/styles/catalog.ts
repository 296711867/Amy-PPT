...
function getStylePackageDir(row: StyleRow): string {
  ...
  return row.source === 'builtin'
    ? path.join(root, 'system', row.style)
    : path.join(root, 'user', row.id)
}
...
export async function saveGeneratedStylePreview(
  ...
  if (row.source === 'builtin') {
    ...
    const tempRoot = await fs.promises.mkdtemp(
      path.join(os.tmpdir(), 'amy-ppt-style-preview-save-')
    )
  ...
  const tempRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'amy-ppt-style-preview-save-'))
  const tempDir = path.join(tempRoot, id)
...
async function writeUserStylePackage(
  ...
  if (!skillMarkdown) {
    throw new Error(
      formatUserStylePackageBackfillError(row.id, 'style_skill 为空，无法生成 SKILL.md')
    )
  }
  const dir = options.rootPath
    ? path.join(options.rootPath, 'user', row.id)
    : getUserStyleDir(row.id)
...
export async function importStylePackageZip(
  ...
  const tempRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'amy-ppt-style-import-'))
...
export async function exportStylePackageZip(
  styleId: string,
  outputPath: string
...
function normalizeStylePackageZipEntries(files: Record<string, Uint8Array>): {
  ...
  if (entries.length < 2 || entries.length > 3) {
    throw new Error(
      '风格包 ZIP 必须只包含一个目录，目录内必须有 style.json、SKILL.md，可选 preview.html。'
    )
  }
