...
import { requireSlideSizeFromHtml, type SlideSizePreset } from '@shared/slide-size'
...
const findSlidePackZipInsideZip = (zipData: Uint8Array): Uint8Array | null => {
  ...
  const candidates = Object.entries(files)
    ...
    .filter((entry): entry is { name: string; data: Uint8Array } => {
      if (!entry.name || isIgnoredArchivePath(entry.name, { allowPresentationFiles: true }))
        return false
...
const extractZipToDirectory = async (
  ...
  for (const [rawName, data] of Object.entries(files)) {
    ...
    if (totalBytes > MAX_EXTRACTED_BYTES)
      throw new Error('导入包解压后体积过大，请精简素材后重试。')
  ...
  const sessionRoot =
    mode === 'single-session-directory' ? path.join(targetDir, Array.from(rootNames)[0]) : targetDir
...
const validateSessionRoot = async (
  ...
  Array<{ pageNumber: number; fileSlug: string; title: string; htmlFileName: string }>
> => {
  ...
  const pages = pagesData.map((page, index) => {
    ...
    if (!htmlPath.toLowerCase().endsWith('.html'))
      throw new Error(`页面文件不是 HTML：${htmlFileName}`)
...
export async function importSessionFile(
  ...
  const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'amy-ppt-session-import-'))
  ...
  try {
    ...
    const title = await readTitleFromIndex(
      path.join(prepared.sessionRoot, 'index.html'),
      fallbackTitle
    )
