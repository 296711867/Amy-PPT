...
const readChartSkillDocs = async (): Promise<ChartSkillDocs> => {
  ...
  const skillRoot = path.join(resolveBuiltinSkillsSourcePath(), 'amy-ppt-chart')
...
export const buildPptxChartRewriteSystemPrompt = (
  docs: ChartSkillDocs
): string => `You are the dedicated PPTX chart parsing agent for Amy-PPT.
...
<amy-ppt-chart/SKILL.md>
${docs.skill}
</amy-ppt-chart/SKILL.md>

<amy-ppt-chart/references/chart.md>
${docs.reference}
</amy-ppt-chart/references/chart.md>
