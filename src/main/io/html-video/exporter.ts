...
export const VIDEO_EXPORT_EVEN_DIMENSIONS_FILTER = 'scale=ceil(iw/2)*2:ceil(ih/2)*2,setsar=1'
...
(async () => {
  ...
  const existing = document.getElementById('amy-ppt-video-static-export');
  if (existing) existing.remove();
  const style = document.createElement('style');
  style.id = 'amy-ppt-video-static-export';
  style.textContent = [
...
(async () => {
  ...
  const style = document.getElementById('amy-ppt-video-animated-export') || document.createElement('style');
  style.id = 'amy-ppt-video-animated-export';
  style.textContent = [
    'html { scroll-behavior: auto !important; }',
    '*, *::before, *::after { transition: none !important; animation: none !important; transition-delay: 0s !important; animation-delay: 0s !important; }'
  ].join('\\n');
...
const writeCapturedFrame = async (args: {
  ...
  const imagePath = path.join(
    args.frameDir,
    `frame-${String(args.frameIndex).padStart(6, '0')}.png`
  )
...
export const exportHtmlPagesToVideo = async (
  ...
  const tempRootDir = path.join(options.tempRootDir || os.tmpdir(), '.amy-ppt-tmp')
  ...
  try {
    ...
    if (
      !is.dev ||
      (process.env.AMY_PPT_KEEP_VIDEO_EXPORT_TMP !== '1' &&
        process.env.OHMYPPT_KEEP_VIDEO_EXPORT_TMP !== '1')
    ) {
