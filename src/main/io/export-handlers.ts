...
<dict>
  ...
  <string>com.amyppt.slidepack.${sanitizeMacBundleIdentifierPart(appName)}</string>
...
const writeMacAppZip = (
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'amy-ppt-slide-pack-app-'))
  try {
  ...
  ipcMain.handle('export:pdf', async (event, payload: unknown) => {
    ...
    const sessionTitle =
      typeof session.title === 'string' && session.title.trim().length > 0
        ? session.title.trim()
        : `amy-ppt-${sessionId}`
    const sanitizedBaseName = sanitizeExportBaseName(sessionTitle, `amy-ppt-${sessionId}`)
  ...
  ipcMain.handle('export:outlinesMarkdown', async (event, payload: unknown) => {
    ...
    const rawTitle =
      typeof session.title === 'string' && session.title.trim().length > 0
        ? session.title.trim()
        : `amy-ppt-${sessionId}`
    const baseName = sanitizeExportBaseName(`${rawTitle}-大纲`, `amy-ppt-${sessionId}-outline`)
    ...
    try {
      ...
      const rawTitle =
        typeof session.title === 'string' && session.title.trim()
          ? session.title.trim()
          : `amy-ppt-${sessionId}`
      const sessionName = sanitizeExportBaseName(rawTitle, `amy-ppt-${sessionId}`)
      ...
      const zipRootName = `amy-ppt-session-${sessionName}`
      const zipFiles: Record<string, Uint8Array> = {}
