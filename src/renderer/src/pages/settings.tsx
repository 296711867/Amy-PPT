...
import { LayoutRulesSettingsTab } from '../components/settings/LayoutRulesSettingsTab'
import {
...
export function SettingsPage(): React.JSX.Element {
  ...
  const initialTab = ['general', 'model', 'imageModel', 'layoutRules', 'advanced'].includes(
    requestedTab || ''
  )
  ...
  const handleThemeChange = async (theme: UiThemeId): Promise<void> => {
    await setUiTheme(theme)
    const saveError = useSettingsStore.getState().verificationMessage
    if (saveError) {
      error(t('settings.themeSaveFailed'), { description: saveError })
      return
    }
    success(t('settings.themeUpdated'))
  }
  ...
  return (
    <div className="mx-auto w-full min-w-0 max-w-4xl p-6">
      <div className="mb-5">
        ...
        <h1 className="organic-serif mt-2 text-[32px] font-semibold leading-none text-foreground">
          {t('settings.title')}
        </h1>
      </div>
      ...
      <Tabs defaultValue={initialTab}>
        <TabsList className="max-w-full justify-start overflow-x-auto">
          ...
          <TabsTrigger value="imageModel" className="shrink-0">
            {t('settings.imageModelTab')}
          </TabsTrigger>
          <TabsTrigger value="layoutRules" className="shrink-0">
            {t('settings.layoutRulesTab')}
          </TabsTrigger>
        ...
        <TabsContent value="layoutRules">
          <LayoutRulesSettingsTab />
        </TabsContent>
      </Tabs>
