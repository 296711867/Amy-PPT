    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger asChild>
        <button
          ...
          disabled={disabled}
          className={cn(
            'flex w-full items-center justify-between gap-2 rounded-lg border border-border bg-card py-2.5 pl-3 text-sm text-foreground shadow-[inset_0_1px_2px_rgb(var(--ui-shadow-color)/0.08)] focus:outline-none focus:ring-2 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50',
            ...
            {selected ? (
              <>
                ...
                {selected.styleCase && !compact && (
                  <span className="hidden shrink-0 truncate rounded-md border border-border bg-secondary px-1.5 py-px text-[10px] font-medium leading-tight text-muted-foreground sm:inline-block">
      ...
      <PopoverContent
        ...
        className={cn(
          'min-w-[var(--radix-popover-trigger-width)] w-[360px] overflow-hidden rounded-lg border border-border bg-[var(--ui-surface-solid)] p-0 text-foreground shadow-[0_12px_28px_rgb(var(--ui-shadow-color)/0.18)]',
          dropdownClassName
        )}
      >
            ...
            filtered.map((option) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'relative flex w-full items-stretch gap-2.5 rounded-md px-2.5 py-2 text-left outline-none transition-colors hover:bg-accent focus-visible:bg-accent',
                ...
                >
                  {thumb ? (
                    ...
                    <span className="h-11 w-[78px] shrink-0 rounded-[3px] border border-border bg-secondary" />
                  )}
                  <div className="flex min-w-0 flex-1 flex-col justify-center gap-0.5 py-0.5">
                    <span className="flex min-w-0 items-center gap-1.5">
                      ...
                      {option.styleCase && (
                        <span className="shrink-0 truncate rounded-md border border-border bg-secondary px-1.5 py-px text-[10px] font-medium leading-tight text-muted-foreground">
                          {option.styleCase}
                        </span>
                      )}
