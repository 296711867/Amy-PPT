...
function deserializeNode(
  node: Node,
  marks: Partial<RichTextLeaf> = {}
): Array<RichTextLeaf | RichTextNode> {
  ...
  return [
    {
      type: 'paragraph',
      children:
        children.length > 0 ? (children as Array<RichTextLeaf | RichTextNode>) : [leafFromText('')]
    }
  ]
}
...
function ColorMarkButton({
  ...
  return (
    <label
      className="relative inline-flex h-6 w-6 cursor-pointer items-center justify-center rounded-md border border-border bg-card/70 transition-colors hover:bg-accent"
      title="Text color"
...
function FontSizeMarkInput({ defaultFontSize }: { defaultFontSize?: string }): React.JSX.Element {
  ...
  return (
    <input
      ...
      onFocus={captureSelection}
      onChange={(event) =>
        setFontSizeMark(editor, Number(event.target.value), selectionRef.current)
      }
      className="h-6 w-[58px] rounded-md border border-border bg-card/70 px-1 text-[11px] text-foreground outline-none focus:border-ring"
    />
  )
}
...
function ToolbarButton({
  ...
  label,
  icon: Icon
}: (typeof commandButtons)[number]): React.JSX.Element {
  const editor = useSlate()
...
export function RichTextBox({
    ...
    return (
      <span
        ...
        style={{
          ...
          textShadow: props.leaf.color
            ? getTextPreviewShadow(props.leaf.color) || 'none'
            : undefined
        }}
      >
        {children}
  ...
  return (
    <div
      ...
      className="overflow-hidden rounded-lg border border-border bg-card/90 shadow-[inset_0_1px_2px_rgb(var(--ui-shadow-color)/0.05)]"
    >
      <Slate
        key={revision}
        editor={editor}
      ...
      >
        <div className="flex h-8 items-center gap-1 border-b border-border bg-secondary/80 px-2">
        ...
        <Editable
          ...
          className={cn(
            'min-h-[120px] overflow-auto px-3 py-2 outline-none selection:bg-accent selection:text-foreground focus-visible:bg-card/60 [&_a]:underline [&_b]:font-bold [&_strong]:font-bold [&_i]:italic [&_em]:italic [&_u]:underline',
            className
          )}
        />
      </Slate>
