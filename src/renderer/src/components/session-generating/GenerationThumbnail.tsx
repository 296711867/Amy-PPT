...
export function GenerationThumbnail({
  ...
  if (!slideSize) {
    return <div className="h-[195px] w-full rounded-xl bg-background/88" />
  }
  ...
  return (
    <div
      className={cn(
        'group relative flex w-full min-w-0 flex-col overflow-hidden rounded-xl border bg-[var(--ui-surface-elevated)]/78 p-1.5 shadow-[0_16px_34px_rgba(70,82,58,0.12)] transition-all duration-500',
        page.status === 'completed' && 'border-[var(--ui-workspace-border)] translate-y-0 opacity-100',
        page.status === 'generating' &&
          'border-[var(--ui-focus)] bg-[var(--ui-selected)]/88 shadow-[0_18px_40px_rgba(95,132,72,0.22)]',
        page.status === 'failed' && 'border-[var(--ui-danger)]/40 bg-[var(--ui-danger-soft)]',
        page.status === 'pending' && 'border-[var(--ui-border-strong)]/72 opacity-72'
      )}
    >
      <div
        className="relative flex h-[180px] w-full min-w-0 shrink-0 items-center justify-center overflow-hidden bg-background/88 shadow-[0_5px_14px_rgba(93,107,77,0.08)]"
        style={{ contain: 'paint' }}
      >
        ...
        {page.status === 'generating' && (
          <div className="absolute inset-0 border-2 border-[var(--ui-focus)]/70">
            <div className="absolute right-2 top-2 rounded-full bg-[var(--ui-surface-elevated)]/90 p-1 shadow-sm">
        ...
        {page.status === 'failed' && (
          <div className="absolute inset-0 flex items-center justify-center bg-[var(--ui-danger-soft)]">
      ...
      <div className="flex w-full min-w-0 items-center justify-between gap-2 px-0.5 pb-0.5 pt-2">
        ...
        {page.status !== 'failed' && (
          <span className="min-w-0 truncate text-xs font-medium text-foreground" title={page.title}>
            {page.title}
          </span>
        )}
      </div>
