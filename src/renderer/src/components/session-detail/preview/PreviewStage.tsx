...
>(function PreviewStage(
  ...
  return (
    <main className="flex min-h-0 flex-1 flex-col px-3 pb-3 pt-1">
      <div className="relative min-h-0 flex-1 overflow-hidden rounded-lg bg-secondary/70 p-3 shadow-[0_18px_38px_rgb(var(--ui-shadow-color)/0.11)]">
        {selectedPage ? (
          <div
            ref={frameRef}
            className="relative h-full overflow-hidden rounded-md bg-background shadow-[0_10px_24px_rgb(var(--ui-shadow-color)/0.11)]"
          >
            {isGeneratingPlaceholder ? (
              <div
                className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-card text-center text-primary"
                aria-live="polite"
              >
            ...
            {isStyleSwitchPageRunning && (
              <div
                className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-3 bg-background/75 text-center text-primary backdrop-blur-[1px]"
                aria-live="polite"
              >
                <Loader2 className="h-7 w-7 animate-spin" />
                <div className="max-w-sm px-6 text-sm font-medium">
            ...
            {pageEditJob && isPageEditing && (
              <div
                className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-3 bg-background/75 text-center text-primary backdrop-blur-[1px]"
                aria-live="polite"
              >
                <Loader2 className="h-7 w-7 animate-spin" />
                <div className="max-w-sm px-6 text-sm font-medium">
            ...
            {selectedPage.status === 'failed' && (
              <div className="absolute bottom-5 left-5 z-20 max-w-[520px] rounded-md bg-[var(--ui-danger-soft)] px-3 py-2 text-xs text-destructive shadow-[0_10px_24px_rgb(var(--ui-shadow-color)/0.12)] backdrop-blur-sm">
                {t('sessionDetail.failedPageHint')}
              </div>
            )}
          </div>
        ) : (
          <div className="relative flex h-full min-h-[420px] flex-col items-center justify-center gap-4 rounded-md bg-background/85 text-center text-primary shadow-[inset_0_0_0_1px_var(--ui-border)]">
