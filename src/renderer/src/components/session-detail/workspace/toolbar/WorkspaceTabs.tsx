import { Eye, LayoutGrid, Palette, Pencil, ScrollText, Sparkles, WandSparkles } from 'lucide-react'
import { cn } from '@renderer/lib/utils'
import { useT } from '@renderer/i18n'
import type { SessionWorkspaceTab } from '@renderer/store'
import { MasterWorkbenchPanel } from '../workbench'

export function WorkspaceTabs({
  activeTab,
  disabled,
  onActivate
}: {
  activeTab: SessionWorkspaceTab
  disabled: boolean
  onActivate: (tab: SessionWorkspaceTab) => void
}): React.JSX.Element {
  const t = useT()
  const tabs: Array<{ id: SessionWorkspaceTab; label: string; icon?: React.JSX.Element }> = [
    { id: 'preview', label: t('sessionDetail.previewMode'), icon: <Eye className="h-3 w-3" /> },
    { id: 'edit', label: t('sessionDetail.editMode'), icon: <Pencil className="h-3 w-3" /> },
    { id: 'browse', label: t('sessionDetail.browseMode'), icon: <LayoutGrid className="h-3 w-3" /> },
    { id: 'style', label: t('sessionDetail.styleMode'), icon: <Palette className="h-3 w-3" /> },
    { id: 'animation', label: t('sessionDetail.animationTab'), icon: <WandSparkles className="h-3 w-3" /> },
    { id: 'speech', label: t('sessionDetail.speechScript'), icon: <ScrollText className="h-3 w-3" /> },
    { id: 'ai', label: t('sessionDetail.aiMode'), icon: <Sparkles className="h-3 w-3" /> }
  ]

  return (
    <div
      className={cn(
        'flex min-w-0 flex-1 justify-center',
        disabled && 'pointer-events-none grayscale opacity-45'
      )}
      aria-disabled={disabled || undefined}
      >
        <div className="flex max-w-full items-center gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <div className="flex shrink-0 items-center gap-0.5 rounded-full bg-[var(--ui-workspace-selected-soft)]/45 p-0.5 shadow-[inset_0_1px_4px_rgb(var(--ui-workspace-shadow-color)/0.08)]">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                type="button"
                className={cn(
                  'inline-flex h-6 min-w-[68px] shrink-0 items-center justify-center gap-1 rounded-full px-2 text-[10px] font-bold leading-none transition-all',
                  activeTab === tab.id
                    ? 'bg-[var(--ui-workspace-selected)] text-[var(--ui-workspace-on-selected)] shadow-[0_4px_10px_rgb(var(--ui-workspace-shadow-color)/0.16)]'
                    : 'text-[var(--ui-workspace-text-muted)] hover:bg-[var(--ui-workspace-surface)]/70 hover:text-[var(--ui-workspace-text)]'
                )}
                onClick={() => onActivate(tab.id)}
                disabled={disabled}
              >
                {tab.icon}
                {tab.label}
              </button>
            ))}
          </div>
          <MasterWorkbenchPanel />
        </div>
    </div>
  )
}
