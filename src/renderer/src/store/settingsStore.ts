...
interface SettingsStore {
  ...
  setUiTheme: (theme: UiThemeId) => Promise<void>
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  fetchSettings: async () => {
    try {
      ...
      const theme = normalizeUiThemeId(typedSettings.theme)
      applyUiTheme(theme)
      set({
        settings: {
          ...typedSettings,
  ...
  setUiTheme: async (value) => {
    const previousTheme = get().settings?.theme
    ...
    try {
      await ipc.saveSettings({ theme })
    } catch (error) {
      if (previousTheme) applyUiTheme(previousTheme)
      set((state) => ({
        settings:
          state.settings && previousTheme
            ? { ...state.settings, theme: previousTheme }
            : state.settings,
        verificationMessage:
          error instanceof Error && error.message
            ? error.message
            : fallbackMessage('保存主题失败。', 'Failed to save theme.')
      }))
    }
  },
