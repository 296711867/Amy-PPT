...
const normalizeMasterElementSize = (
  value: unknown,
  fallback: MasterElementSize
): MasterElementSize => {
  const record = isRecord(value) ? value : {}
...
export function normalizeMasterElementsConfig(value: unknown): MasterElementsConfig {
  ...
  const footerSize = normalizeMasterElementSize(
    record.footerSize,
    DEFAULT_MASTER_ELEMENTS.footerSize
  )
  ...
  const pageNumberPosition = hasPageNumberSize
    ? normalizeMasterElementPosition(
        record.pageNumberPosition,
        DEFAULT_MASTER_ELEMENTS.pageNumberPosition
      )
    : normalizeLegacyMasterElementPosition(
        record.pageNumberPosition,
        { x: 96, y: 96 },
  ...
  const watermarkPosition = hasWatermarkSize
    ? normalizeMasterElementPosition(
        ...
        DEFAULT_MASTER_ELEMENTS.watermarkPosition
      )
    : normalizeLegacyMasterElementPosition(
        record.watermarkPosition,
        { x: 50, y: 50 },
...
export function buildMasterCss(value: unknown, fontFaceCss = ''): string {
  ...
  return `/* Amy-PPT Slide Master. Managed by the application. */\n${fontFaces ? `${fontFaces}\n` : ''}:root {\n${variables.join(
    '\n'
...
export function buildMasterElementsHtml(value: unknown): string {
  ...
  return `<!-- Amy-PPT Slide Master elements. Managed by the application. -->
